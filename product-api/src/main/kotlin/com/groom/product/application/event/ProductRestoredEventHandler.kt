...
/**
 * ProductRestoredEvent 핸들러.
 *
 * 상품 복원 이벤트를 처리하여 감사 로그를 생성합니다.
 * - @TransactionalEventListener(AFTER_COMMIT): 메인 트랜잭션 성공 후 실행
 * - @Transactional(REQUIRES_NEW): 독립적인 새 트랜잭션에서 실행
 * - 이벤트 핸들러 실패해도 메인 작업(상품 복원)은 성공 보장
 */
@Component
class ProductRestoredEventHandler(
    private val productAuditRepository: ProductAuditJpaRepository,
) {
    @TransactionalEventListener(phase = TransactionPhase.AFTER_COMMIT)
    @Transactional(propagation = Propagation.REQUIRES_NEW)
    fun handleProductRestored(event: ProductRestoredEvent) {
        val changeSummary = "상품 복원: '${event.name}'"
